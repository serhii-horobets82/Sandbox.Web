      ...
      <v-card flat style="background-color: #F7FBFF">
        ...
        <v-card-text style="min-height: 150px">
          <table class="skillHeaderRow">
            <tbody>
              <tr>
                <td style="width: 50%">Current competence level</td>
      ...
      <v-card flat style="background-color: #F7FBFF">
        <v-card-text style="min-height: 150px">
          <table class="skillHeaderRow">
            <tbody>
              <tr>
                <td style="width: 50%">Required skills</td>
                <td v-for="i in [1,2,3,4,5]" :key="i" class="text-xs-center">{{ `E${i}` }}</td>
              </tr>
            </tbody>
          </table>
          <table class="skillRow" v-for="competence in competences" :key="competence.id">
            <tbody>
              <tr>
                <td style="width: 50%">{{ competence.name }}</td>
                <td
                  v-for="i in [1,2,3,4,5]"
                  :key="i"
                  class="competenceLevelFill text-xs-center"
                ...
                >
                  ...
                  <v-icon
                    style="color: #B5D6EC"
                    small
                    v-if="competence.roleLevel === i"
                  >error_outlined</v-icon>
                </td>
              </tr>
            </tbody>
          </table>
