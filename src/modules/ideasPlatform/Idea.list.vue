<template>
  <v-container class="px-0">
    <v-layout row wrap class="mb-3">
      <span class="title font-weight-bold">Ideas Platform</span>
    </v-layout>

    <!-- <v-container grid-list-xl class="pa-0 ma-0"> -->

    <v-layout row wrap>
      <v-flex xs6  style="padding-right: 25px;">
        <v-tabs
          class="tabs"
          v-model="tabsModel"
          ...
          slider-color="#3DB3ED"
        >
          <v-tab class="active-class" v-for="(tab, i) in tabs" :key="i+'-tab'">
            {{ tab.name }}
          </v-tab>
        </v-tabs>

        <v-card v-for="idea in ideasFiltered" :key="idea.id+'-idea'"
          class="mt-3 pa-4"
          flat
          style="border: 1px solid #E8EFF7;border-radius: 2px;">
          <v-layout row wrap>
            <v-flex xs12>
              <v-card flat>
                <v-card-title class="pa-0">
                  <span class="mr-5" style="font-size: 13px;">{{ idea.createdBy.nameTemp }}</span>
                  <span style="font-size: 13px; opacity: 0.5;">{{ idea.createdDate | formatDateHuman }}</span>
                  <v-spacer></v-spacer>
            ...
            <v-flex xs12 class="mt-2 mb-2">
              <router-link
                ...
                :to="{name: 'ideas-platform-view', params: {id: idea.id}}"
              >
                {{idea.name}}
              </router-link>
            </v-flex>
            ...
            <v-flex xs12 class="mt-3">
              ...
              <v-spacer></v-spacer>
              <!-- <v-btn class="ma-0" flat small style="border: 1px solid #E8EFF7; border-radius: 70px; color: #ABC1CF !important; font-size: 12px;">
                <svg class="mr-2" width="19" height="12" viewBox="0 0 19 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M11.5555 0L18.6666 5.55556L11.5555 11.1111V8.22221C11.1772 8.20444 4.58286 7.99256 0 12C1.834 6.34947 7.59417 3.89173 11.5555 2.88889V0Z" fill="#D8E3EE"/>
                </svg>
                Share
              </v-btn> -->
                </v-card-title>
      ...
      <v-flex xs6>
        <v-card flat style="border: 1px solid #E8EFF7;border-radius: 2px;">
        ...
        <v-list two-line class="py-0 px-2">
          <template v-for="(strategy, i) in companyStrategy">
            <v-list-tile style="padding-top: 5px; padding-bottom: 5px" :key="strategy.id+'-strategy'">
              <v-list-tile-avatar size="54" class="mr-2">
                <div v-if="i == 0"
                  style="height: 100%; width: 100%; line-height: 54px;
                    color: #3DB3ED !important; background-color: rgba(61,179,237, 0.2);
                    font-size: 20px; font-weight: bold">
                {{strategy.number}}
                </div>
                <div v-if="i == 1"
                  style="height: 100%; width: 100%; line-height: 54px;
                    color: #FFB800 !important; background-color: rgba(255,184,0, 0.2);
                    font-size: 20px; font-weight: bold">
                {{strategy.number}}
                </div>
                <div v-if="i == 2"
                  style="height: 100%; width: 100%; line-height: 54px;
                    color: #939ED7 !important; background-color: rgba(147,158,215, 0.2);
                    font-size: 20px; font-weight: bold">
                {{strategy.number}}
                </div>
              </v-list-tile-avatar>
              ...
              <v-list-tile-action>
                ...
                <path d="M13 19.482H25.9636" stroke="#3C88B5" stroke-linecap="round"/>
                <path d="M19.4817 13V25.9636" stroke="#3C88B5" stroke-linecap="round"/>
                </svg>

              </v-list-tile-action>
            </v-list-tile>

            <v-divider v-if="i < companyStrategy.length -1" :key="i"
        ...
        <v-card flat height="136"
          :to="{ name: 'ideas-platform-edit', params: { id: 0 }}"
          style="border: 1px dashed #CCD9E8; padding-top: 32px; background-color: #f5f9fc;"
          class="mt-4 text-xs-center"
          >
          <svg  width="31" height="42" viewBox="0 0 31 42" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M10.3336 37.8896V41.3342H20.6672V37.8896H10.3336Z" fill="#D2DFED"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M15.4433 8.61145C9.4978 8.63728 4.32124 12.7359 2.93334 18.5144C1.64758 23.8676 4.07351 28.9907 8.60806 31.8451L8.59773 36.1676H22.3859C22.3876 34.7245 22.3928 33.2815 22.3962 31.8384C26.9238 28.9419 29.3832 23.7651 28.044 18.4001C26.6047 12.634 21.3885 8.58562 15.4433 8.61145ZM15.4605 12.056C19.8385 12.037 23.6418 14.9915 24.7008 19.2343C25.7631 23.49 23.824 27.3981 19.8704 29.477L18.9521 29.9581L18.9487 32.7231H12.0496L12.0564 29.9513L11.1213 29.4703C7.14888 27.4263 5.25499 23.6034 6.28418 19.3184C7.30541 15.0665 11.0829 12.0754 15.4606 12.056H15.4605Z" fill="#D2DFED"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M13.7778 0V5.16678H17.2224V0H13.7778Z" fill="#D2DFED"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M28.5613 3.74023L24.9082 7.39331L27.3436 9.82869L31 6.17562L28.5613 3.74023Z" fill="#D2DFED"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M2.43538 3.74023L0 6.17562L3.65307 9.82869L6.08847 7.39331L2.43538 3.74023Z" fill="#D2DFED"/>
          </svg>
          <div>
          + Create Idea
          </div>
  ...
  data: () => ({
    ...
    items: [
      ...
      { title: "I Evaluate", url: "personal-i-evaluate" },
      {
        ...
        url: "personal-suggestions-to-improve"
      },
  ...
  computed: {
    ideasFiltered() {
      ...
      switch(this.tabsModel){
        ...
        case 2:
          return [...this.ideas].sort((a, b) => parseFloat(a.createdDate) - parseFloat(b.createdDate));
        case 3:
          return [...this.ideas].sort((a, b) => parseFloat(a.ideaComment.length) - parseFloat(b.ideaComment.length));
        case 4:
...
.v-tabs__container .v-tabs__item {
