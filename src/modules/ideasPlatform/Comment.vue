    ...
    <v-flex xs12>
      <v-card flat class="ma-2">
        ...
        <v-card-actions class="px-3">
          ...
          <span style="color: rgba(60,136,181,0.5); font-size: 13px">{{comment.createdDate}}</span>
        </v-card-actions>
      </v-card>
    </v-flex>

    <v-layout row wrap  :style="{'margin-left': `25px`}">
      <Comment v-for="c in comment.comments" :key="c.id" :comment="c" :level="level+1"></Comment>
    </v-layout>
  </v-layout>
</template>
