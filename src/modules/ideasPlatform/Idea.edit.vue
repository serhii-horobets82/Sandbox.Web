<template>
  <v-container >
    <v-layout row wrap class="mb-3">
      <span class="title font-weight-bold" style="line-height: 48px !important;">
        {{this.id === 0 ? 'Create New Idea' : 'Edit Idea'}}
      </span>
      <v-spacer></v-spacer>
      <v-btn
        style="background: #3C88B5; border-radius: 60px; color: white; padding: 0 32px"
        class="mr-3"
        @click="save()"
      >Post Idea</v-btn>
      ...
      <v-flex xs12>
        <v-layout row>
          <!-- <v-flex xs12> -->
            ...
            <v-text-field label="Tags" @keypress.enter="addTag()" v-model="tagInput"
            ></v-text-field>
          <!-- </v-flex>
          <v-flex> -->
            <v-spacer></v-spacer>
...
//       };
      this.idea = Object.assign({}, this.idea, {ideaTagRef: []})
    } else {
      const res = await axios.get(this.$backendUrl + `api/ideas/${this.id}`);
      this.idea = res.data;
      // this.idea.ideaTagRef.push({tagId: 1}, {tagId: 2})
    }
    const res = await axios.get(this.$backendUrl + `api/ideaTags`);
    this.tagsById = Object.assign({}, this.tagsById, res.data.reduce((obj, curr) => {obj[curr.id] = curr.name; return obj;}, {}));
    this.tagsByName = Object.assign({}, this.tagsByName, res.data.reduce((obj, curr) => {obj[curr.name.toLowerCase()] = curr.id; return obj}, {}));
  },

  methods: {
    async addTag() {
      let id = this.tagsByName[this.tagInput.toLowerCase()];
      if (!id) {
        const res = await axios.post(this.$backendUrl + `api/ideaTags`, {name: this.tagInput});
        this.idea.ideaTagRef.push({tagId: res.data.id});

        this.tagsById[res.data.id] = res.data.name;
        this.tagsByName[res.data.name] = res.data.id;
      } else {
        this.idea.ideaTagRef.push({tagId: id});
      }
    ...
    async save(){
      if (this.id === 0) {
        const res = await axios.post(this.$backendUrl + `api/ideas`, this.idea);
      } else {
        const res = await axios.put(this.$backendUrl + `api/ideas/${this.id}`, this.idea);
      }
      toast.success(`Idea has been ${this.id === 0 ? 'created': 'saved'}.`)
      this.$router.push({name: 'ideas-platform-list'})
    },
  }
}
</script>
